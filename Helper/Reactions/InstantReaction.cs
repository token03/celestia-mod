...
namespace Celestia.Helper.Reactions
{
	public class InstantReaction
	{
		...
		{
			double strikeDamage = npc.StrikeNPC(Convert.ToInt32(damage / 2), 0, 0, true); // converts to crit for safety reasons
			...
			player.addDPS(Convert.ToInt32(strikeDamage)); // for dpsmeter
		}
	}
}
